    ...
    def chew(self, line):
        ...
        if line in armor_header_lines:
            if self.__dict__.has_key(
                    'title') and 'SIGNED MESSAGE' == self.title and '-----BEGIN PGP SIGNATURE-----' == line:
        ...
        if 'SIGNED MESSAGE' == self.title:
            ...
            if dsum == self.chksum:
                ...
                raise PGPFormatError(
                    "Data did not match chksum. Received chksum->(%s), Calculated chksum->(%s))" % (self.chksum, dsum))
...
def list_armored(d):
    ...
    for line in [x.rstrip() for x in d.replace("\r\n","\n").split("\n")]:
            ...
            if len_text_sigs and MSG_LITERAL == target.msg.type and len_text_sigs == len(target.sigs):
                ...
                for text_sig in text_sigs:
                    ...
                    if alg_hash != text_sig.body.alg_hash:
                        raise PGPFormatError(
                            "Clearsigs require the same hash for each signature. 1st sig hash->(%s) conflicts with a following sig->(%s)" % (
                            alg_hash, text_sig.body.alg_hash))
